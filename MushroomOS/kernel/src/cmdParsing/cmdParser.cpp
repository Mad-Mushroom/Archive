...
void ParseCommand(char* input, char* oldInput, OSUser** user, Window* window)
{
    ...
    if (StrEquals(input, "clear") || StrEquals(input, "cls"))
    {
    ...
    if (StrEquals(input, "howmanylines"))
    {
        Println(window, "\n23.379 Lines of Code.");
        Println(window, "In version 0.4.8");
        RemoveFromStack();
        return;
    }

    if (StrEquals(input, "sysinfo") || StrEquals(input, "ver"))
    {
        Println(window, "\nMushroomOS [verison 0.4.8]");
        RemoveFromStack();
        return;
    }
    ...
    if (StrEquals(input, "m") || StrEquals(input, "manager"))
    {
        {
            Window* oldActive = activeWindow;
            Window* pmWindow = (Window*)malloc(sizeof(Window), "ProgramMenu Window");
            ProgramMenuInstance* pm = (ProgramMenuInstance*)malloc(sizeof(ProgramMenuInstance), "ProgramMenu Instance");
            *pm = ProgramMenuInstance(pmWindow);
            *(pmWindow) = Window((DefaultInstance*)pm, Size(200, 200), Position(10, GlobalRenderer->framebuffer->Height-50-200), "Manager", true, true, true);
            osData.windows.add(pmWindow);
            
            pm->Init();
            
            activeWindow = pmWindow;          
            pmWindow->moveToFront = true;
            osData.mainTerminalWindow = pmWindow;
...
void SetCmd(const char* name, const char* val, OSUser** user, Window* window)
{
    ...
    else if (StrEquals(name, "dterminal"))
    {
        if (StrEquals(val, "true") || StrEquals(val, "show"))
        {
        ...
        else if (StrEquals(val, "false") || StrEquals(val, "hide"))
        {
